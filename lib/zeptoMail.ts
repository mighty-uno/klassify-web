...
import { applicationEmailHtml } from "@/lib/emailTemplates";
...
export async function sendApplicationEmail(app: ApplicationRecord): Promise<void> {
  ...
  const response = await fetch("https://api.zeptomail.com/v1.1/email/send", {
    ...
    body: JSON.stringify({
      ...
      subject: `New Bee-Bud application from ${app.name}`,
      htmlbody: applicationEmailHtml(app),
      track_clicks: true
    }),
