...
export function enquiryEmailHtml(enquiry: EnquiryRecord): string {
  const rows: Array<[string, string]> = [
    ["Name", enquiry.name],
    ["Email", enquiry.email],
    ["Mobile", enquiry.mobile],
    ["School", enquiry.school],
    ["School address", enquiry.address]
  ];

  const replyHref = `mailto:${encodeURIComponent(enquiry.email)}`;
  const callHref = `tel:+91${enquiry.mobile}`;
...
<html lang="en">
  <head>
    ...
    <title>New SkooBee enquiry</title>
  </head>
          ...
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e3e3ea;">
              ...
              <td style="background-color:#0d0e1a;padding:26px 32px;">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                  <tr>
                    ...
                    <td align="right" style="color:#ffc93c;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;">Contact</td>
                  </tr>
                </table>
              </td>
            </tr>
            <tr>
              <td style="padding:32px 32px 6px 32px;">
                <h1 style="margin:0;font-size:24px;font-weight:800;color:#14162b;letter-spacing:-0.5px;">New SkooBee enquiry</h1>
                <p style="margin:10px 0 0 0;font-size:15px;line-height:1.65;color:#5e6377;">
                  A visitor has submitted the contact form on the SkooBee website. Follow up with them to talk through their school&apos;s needs.
                </p>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td align="center" style="padding:26px 32px 34px 32px;">
                <a href="${replyHref}" style="display:inline-block;background-color:#4a3aff;color:#ffffff;text-decoration:none;font-size:14px;font-weight:700;padding:13px 28px;border-radius:10px;">Reply to ${escapeHtml(enquiry.name)}</a>
                ${enquiry.mobile ? `&nbsp;<a href="${callHref}" style="display:inline-block;background-color:#ffffff;border:1px solid #c9cbe0;color:#14162b;text-decoration:none;font-size:14px;font-weight:700;padding:12px 28px;border-radius:10px;">Call ${escapeHtml(enquiry.mobile)}</a>` : ""}
              </td>
            </tr>
            <tr>
